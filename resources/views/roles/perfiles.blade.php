...
<div class="main-content">
    <div class="page-content">
        <div class="container-fluid">
                        ...
                        <div class="card-body">
                            <div class="d-flex align-items-center">
                                ...
                                <div class="flex-grow-1 ms-3">
                                    ...
                                    <h4 class=" mb-0"><span>{{ $totalRoles ?? 0 }}</span></h4>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="card-body">
                            <div class="d-flex align-items-center">
                                ...
                                <div class="flex-grow-1 ms-3">
                                    ...
                                    <h4 class=" mb-0"><span>{{ $totalPermisos ?? 0 }}</span></h4>
                                </div>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="table-responsive table-card mb-3">
                                <table class="table align-middle table-nowrap mb-0" id="customerTable">
                                    <thead class="table-light">
                                        <tr>
                                            <th style="font-size: 12px;" data-sort="Profiles" scope="col">Perfiles</th>
                                            <th style="font-size: 12px;" data-sort="Administrator_access" scope="col">Acceso Administrador</th>
                                    ...
                                    <tbody class="list form-check-all">
                                        @if($perfiles->isEmpty())
                                            <tr>
                                                <td class="text-center" colspan="3">Profiles not registered</td>
                                            </tr>
                                        @else
                                            @foreach( $perfiles as $perfil )
                                                <tr>
                                                    <td style="font-size: 12px;">{{ $perfil->role }}</td>
                                                    <td style="font-size: 12px;">{{ ($perfil->acceso_admin == 1) ? 'Con acceso' : 'Sin acceso'; }}</td>
                                                    <td>
                                                        <div class="hstack gap-2">
                                                            <a href="#" class="btn btn-sm btn-soft-primary remove-list" data-bs-toggle="modal" data-bs-target="#editProfiles{{ $perfil->id_role }}"><i class="bx bx-edit align-bottom"></i></a>
                                                        </div>
                                                    </td>
                                                </tr>
                                            @endforeach
            ...
            @foreach ($perfiles as $perfil)
                <div class="modal fadeInLeft" id="editProfiles{{ $perfil->id_role }}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                    <div class="modal-dialog modal-lg modal-dialog-centered">
                        <div class="modal-content border-0">
                            <div class="modal-header bg-soft-info p-3">
                                ...
                                <div class="modal-body">
                                    <div class="row g-3">
                                        <div class="col-lg-12">
                                            ...
                                            <input type="text" id="role" name="role" class="form-control" value="{{ $perfil->role }}" placeholder="Enter Roles" required />
                                            <div class="valid-feedback">
                                                Looks good!
                                            </div>
                                            <div class="invalid-feedback text-end">
                                        ...
                                        <div class="col-lg-12">
                                            <label for="role" class="form-label">Administrator Access</label>
                                            <select id="acceso_admin" name="acceso_admin" class="form-control" required>
                                                <option value="0" @if ($perfil->acceso_admin == 0) selected @endif>Sin acceso</option>
                                                <option value="1" @if ($perfil->acceso_admin == 1) selected @endif>Con acceso</option>
                                            </select>
                                        </div>

                                        @php
                                            $permisos_checked = App\Models\RolePermiso::where('idRole', $perfil->id_role)->pluck('idPermiso')->all();
                                        @endphp

                                        <div class="mb-3" @if ($perfil->acceso_admin == 0) style="display: none;" @endif>
                                            <label class="form-label">Permisos</label>
                                            <div class="permisos">
                                                @if ($perfil->acceso_admin == 1 && $permisos)
            ...
            <script>
                document.addEventListener('DOMContentLoaded', function() {
                    document.querySelectorAll('.marcar-todos').forEach(function(marcarTodos) {
                        marcarTodos.addEventListener('change', function() {
                            const checkStatus = this.checked;

                            this.closest('.permiso').querySelectorAll('.permiso-check').forEach(function(checkbox) {
